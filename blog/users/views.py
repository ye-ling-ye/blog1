...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # 响应注册结果
        #redirect 是进行重定向
        # reserve是可以通过namespace：name来获取到视图对应的路由
...
class ImageCodeView(View):
    ...
    def get(self,request):
        ...
        # 返回响应，将生成的图片以content_type为image/jpeg的形式返回给请求（返回图片二进制）
...
class SmsCodeView(View):
    ...
    def get(self, request):
        ...
        uuid = request.GET.get('uuid') #
        mobile=request.GET.get('mobile') #电话号码
...
#定义用户中心的视图
#LoginRequiredMixin：如果用户未登录的话则会进行默认的跳转
